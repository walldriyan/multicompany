                          ...
                          <div className="p-2 space-y-1 rounded-md bg-red-950 border border-red-800">
                              <Table className="text-xs">
                                  ...
                                  <TableBody>{(saleForReturnLogDisplay?.returnedItemsLog || []).map((logEntry, index) => {
                                        ...
                                        const priceAtSaleForLog = originalItemDetails?.priceAtSale || logEntry.refundAmountPerUnit; 
              ...
              <Card className="bg-card border-border/70 mt-2">
                ...
                <CardContent className="p-3 pt-0">
                  <ScrollArea className="flex-shrink-0 border border-orange-700 rounded-md bg-orange-950/50 min-h-[150px]">
                      <Table><TableHeader><TableRow className="bg-orange-800/60 sticky top-0 z-[1] hover:bg-orange-800/70 border-b-orange-700"><TableHead className="text-orange-200 py-1.5">Item</TableHead><TableHead className="text-center text-orange-200 py-1.5">Qty in Bill</TableHead><TableHead className="text-right text-orange-200 py-1.5">Eff. Price/Unit</TableHead><TableHead className="text-right text-orange-300 font-medium py-1.5">Line Total (Net)</TableHead><TableHead className="w-32 text-center text-orange-200 py-1.5">Qty to Return</TableHead></TableRow></TableHeader>
                        <TableBody>{itemsToReturnUiList.length > 0 ? itemsToReturnUiList.map(item => { 
                            const lineTotalNetInBill = item.effectivePricePaidPerUnit * item.quantity;
                            return (
                            <TableRow key={`ret-item-${item.productId}`} className={`hover:bg-orange-900/60 border-b-orange-800/50 ${item.quantity === 0 ? 'opacity-60' : ''}`}>
                                ...
                                <TableCell className="text-right text-orange-300 py-1">Rs. {item.effectivePricePaidPerUnit.toFixed(2)}</TableCell>
